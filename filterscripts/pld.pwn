...
#include "./pld/zones.pwn"
#include "./pld/cmds.pwn"
...
public CreateTextDraws(playerid) {
	p_BarTextdraw[playerid][0] = CreatePlayerTextDraw(playerid, 124.000000, 401.000000, "");
	PlayerTextDrawTextSize(playerid, p_BarTextdraw[playerid][0], 0.216664, 3.249998);
	...
	PlayerTextDrawFont(playerid, p_BarTextdraw[playerid][0], 1);
	PlayerTextDrawSetProportional(playerid, p_BarTextdraw[playerid][0], 0);

	p_LocationTextdraw[playerid][0] = CreatePlayerTextDraw(playerid, 127.000000, 406.000000, "Unknown_Location");
	PlayerTextDrawLetterSize(playerid, p_LocationTextdraw[playerid][0], 0.248332, 1.000000);
	...
	p_LocationTextdraw[playerid][1] = CreatePlayerTextDraw(playerid, 127.000000, 415.000000, "Undefined,_San_Andreas");
	PlayerTextDrawLetterSize(playerid, p_LocationTextdraw[playerid][1], 0.191666, 1.000000);
	...
	p_DirectionTextdraw[playerid] = CreatePlayerTextDraw(playerid, 114.000000, 402.000000, "N");
	PlayerTextDrawLetterSize(playerid, p_DirectionTextdraw[playerid], 0.308333, 2.549999);
...
public OnPlayerUpdate(playerid) {
	...
	// if(rz >= 146.25 && rz < 168.75) p_TextDirection[playerid] = "SSW";
...
public OnPlayerLeaveDynamicArea(playerid, areaid) {
    if (!IsPlayerInAnyDynamicArea(playerid)) {
		PlayerTextDrawSetString(playerid, p_LocationTextdraw[playerid][0], "No_Signal");
		PlayerTextDrawSetString(playerid, p_LocationTextdraw[playerid][1], "Unknown,_Chicago");
    }
    return 1;
}