...
CMD:takeveh(playerid, params[])
{
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 2.3, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]))
        {
			new found = 0, msg2[512];
			for (new i = 0; i < MAX_VEHICLES; i ++) {
				if(pvData[i][cOwner] == pData[playerid][pID] && pvData[i][cPark] == id)
				{
					gListedItems[playerid][found] = i;
					format(msg2, sizeof(msg2), "%s{00FFFF}%s\n", msg2, GetVehicleModelName(pvData[i][cModel]));
					found++;
				}
			}
			if(found) Dialog_Show(playerid, GarageTakeVeh, DIALOG_STYLE_LIST, "Take Vehicle", msg2, "Select", "Close");
		}
	}
	return 1;
}

Dialog:GarageTakeVeh(playerid, response, listitem, inputtext[]) {
    if(response)
	{
