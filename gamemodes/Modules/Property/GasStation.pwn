...
GStation_Refresh(gsid)
{
	if(gsid != -1)
    {
		...
		format(string, sizeof(string), "[FUEL PUMP: %d]\n"WHITE_E"Price: "LG_E"$%s /liters\n\n"WHITE_E"Type '"RED_E"/fill"WHITE_E"' to refill", gsid, FormatMoney(HargaBensin));
...
{
	if(GetVehicleFuel(GetPlayerVehicleID(playerid)) >= 999.0) return Error(playerid, "This vehicle gas is full.");
	if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return Error(playerid, "Anda harus berada didalam kendaraan");
	foreach(new gsid : GStation)
	{
		if(IsPlayerInRangeOfPoint(playerid, 6.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]))
		{
			new String[212];
			format(String, sizeof(String), "{FFFFFF}Price per gallon: {00FF00}$%s",FormatMoney(HargaBensin));
			Dialog_Show( playerid, RefuelVehicle, DIALOG_STYLE_INPUT, "Buy Gasoline", String, "Fill", "Cancel");
		}
	}
	return 1;
}
...
Dialog:RefuelVehicle(playerid, response, listitem, inputtext[]) {
    if (response)
	{
		...
		new value = idiot * HargaBensin, vehicleid = GetPlayerVehicleID(playerid);
		if(idiot > 1000 || idiot < 0) return Error(playerid, "Tidak bisa di atas 1000");
		...
		new old = GetVehicleFuel(vehicleid);
		SetVehicleFuel(vehicleid, old + idiot);
		if(GetVehicleFuel(vehicleid) >= 999.0) SetVehicleFuel(vehicleid, 1000);
	}
	return 1;
}
