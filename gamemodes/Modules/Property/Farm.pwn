...
Player_OwnsFarms(playerid, index)
{
	if(pData[playerid][pID] == -1)
		return 0;

	if((Iter_Contains(FARMS, index) && laData[index][laOwner] != 0) && laData[index][laOwner] == pData[playerid][pID])
		return 1;
...
CMD:lasafe(playerid)
{
    ...
    {
     	ShowPlayerDialog(playerid, FARM_SAFE, DIALOG_STYLE_LIST, "Farm Safe", "Potato\nWheat\nOrange\nChange Name Farm", "Select", "Cancel");
    }
 	else
   	{
