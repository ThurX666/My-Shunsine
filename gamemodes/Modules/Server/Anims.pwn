...
PlayAnimEx(playerid, animlib[], animname[], Float:fDelta, loop=1, lockx, locky, freeze, time=0, forcesync=1)
{
	...
	CallRemoteFunction("ShowPlayerAnimxtdraw", "d", playerid);
	PlayerTextDrawShow(playerid, AnimTD[playerid]);
}
...
CMD:animlist(playerid, params[])
{
	...
	SendClientMessage(playerid, COLOR_GRAD6, "/lowbodypush /headbutt /airkick /doorkick /leftslap /elbow /coprun /sit /lean /wank");
...
CMD:sitdown(playerid, params[])
	return callcmd::sit(playerid, params);
...
CMD:sit(playerid, params[])
{
	...
	switch(strval(params))
	{
		...
		default: Usage(playerid, "/sit [1-7]");
	}
	return 1;
}
...
CMD:lay(playerid, params[])
{
	...
	case 6: PlayAnimEx(playerid, "BEACH", "bather", 4.0, 1, 0, 0, 0, 0, 1);
	case 7: PlayAnimEx(playerid, "BEACH", "Lay_Bac_Loop", 4.0, 1, 0, 0, 0, 0, 1);
	case 8: PlayAnimEx(playerid, "BEACH", "SitnWait_loop_W", 4.0, 1, 0, 0, 0, 0, 1);
	default: Usage(playerid, "/lay [1-8]");
	}
	return 1;
}
