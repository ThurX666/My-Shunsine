...
public OnDynamicObjectMoved(objectid)
{
	...
	// 	    if (y < X_DOOR_L_OPENED - 0.5)   // Some floor doors have shut, move the elevator to next floor in queue:
	// 	    {
...
Dialog:ElevatorLS(playerid, response, listitem, inputtext[]) {
    if(response) 
	{
...
stock Elevator_MoveToFloor(floorid)
{
	...
	SetTimerEx("Elevator_Boost", 2000, 0, "i", floorid);
...
stock ReadNextFloorInQueue()
{
	...
	SetTimer("ElevatorMoved", 1000, 0);
	return 1;
}

public ElevatorMoved()
{
	new Float:x, Float:y, Float:z;
	GetObjectPos(Obj_FloorDoors[ElevatorFloor][0], x, y, z);
...
stock ShowElevatorDialog(playerid)
{
	...
	Dialog_Show(playerid, ElevatorLS, DIALOG_STYLE_LIST, "Elevator", string, "Select", "Close");
