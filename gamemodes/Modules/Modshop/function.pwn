...
{    
    ...
    else if(VehicleObjects[id][slot][vehObjectType] == OBJECT_TYPE_LIGHT) {
        if(!GetLightStatus(id)) {
            if(IsValidDynamicObject(VehicleObjects[id][slot][vehObject]))
                DestroyDynamicObject(VehicleObjects[id][slot][vehObject]);
            VehicleObjects[id][slot][vehObject] = INVALID_OBJECT_ID;
        }
    }
    else if(VehicleObjects[id][slot][vehObjectType] == OBJECT_TYPE_PLATE) Vehicle_PlateSync(id, slot);
    else if(VehicleObjects[id][slot][vehObjectType] == OBJECT_TYPE_NEON) {
        if(!GetLightStatus(id)) {
            if(IsValidDynamicObject(VehicleObjects[id][slot][vehObject]))
                DestroyDynamicObject(VehicleObjects[id][slot][vehObject]);
            VehicleObjects[id][slot][vehObject] = INVALID_OBJECT_ID;
